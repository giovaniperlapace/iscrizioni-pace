-- Milestone P4: atomic single/batch publication and safe published-panel updates.
...
begin
  ...
  loop
    if not exists (
      ...
      raise exception 'panel % is incomplete or has an invalid capacity total', v_panel_id
        using errcode = '23514';
    end if;
  end loop;
...
begin
  ...
  if v_section_capacity <> v_location_capacity then
    raise exception 'panel section capacity total must equal location capacity'
      using errcode = '23514';
  end if;
...
revoke all on function public.publish_panels(uuid, uuid[]) from public;
grant execute on function public.publish_panels(uuid, uuid[]) to authenticated;

revoke all on function public.save_published_panel(
  uuid, uuid, text, text, uuid, timestamptz, timestamptz, jsonb
) from public;
grant execute on function public.save_published_panel(
  uuid, uuid, text, text, uuid, timestamptz, timestamptz, jsonb
) to authenticated;

comment on function public.publish_panels(uuid, uuid[])
  is 'Idempotently publishes one or more valid draft panels in one transaction.';

comment on function public.save_published_panel(
  uuid, uuid, text, text, uuid, timestamptz, timestamptz, jsonb
) is 'Atomically updates a published panel, its sections and impact audit.';
