...
// Only locks an explicit native submission; never submits or consumes a token itself.
export const CONFIRMATION_SUBMIT_SCRIPT = `(() => {
...
const submitScriptHash = createHash("sha256").update(CONFIRMATION_SUBMIT_SCRIPT).digest("base64");

// No auto-submit, prefetch, or link to a token-consuming GET.
...
export const MAGIC_LINK_RESPONSE_HEADERS = {
  ...
  "Content-Security-Policy": `default-src 'none'; style-src 'unsafe-inline'; form-action 'self'; base-uri 'none'; frame-ancestors 'none'; script-src 'sha256-${submitScriptHash}'`,
