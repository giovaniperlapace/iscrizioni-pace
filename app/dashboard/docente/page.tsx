...
export default async function TeacherDashboardPage({ searchParams }: Props) {
  ...
  return <main className="app-page text-[var(--peace-ink)]"><section className="event-gradient py-9 text-white"><div className="app-container"><p className="text-xs font-bold uppercase tracking-[0.14em] text-white/75">{copy.eyebrow}</p><h1 className="mt-2 text-3xl font-semibold">{copy.manage}</h1><p className="mt-3 text-white/80">{user.email}</p></div></section><section className="app-container grid gap-6 py-8">
    {params.saved ? <Status>Modifiche salvate.</Status> : null}{params.cancelled ? <Status>Prenotazione annullata e posti liberati.</Status> : null}{params.error ? <p role="alert" className="rounded-md border border-[#d9a99d] bg-[#fff0eb] px-4 py-3 text-sm text-[#7f2f20]">{params.error === "capacity" ? copy.capacity : params.error === "overlap" ? copy.overlap : copy.invalid}</p> : null}
    <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm leading-6 text-[var(--peace-muted)]">Ogni scheda corrisponde a una classe o a un gruppo. Le prenotazioni annullate restano consultabili.</p><Link href="/scuole" className="inline-flex min-h-11 items-center rounded-md bg-[var(--peace-blue-800)] px-4 font-semibold text-white">{copy.another}</Link></div>
    {catalog.bookings.length === 0 ? <p className="rounded-xl border border-dashed border-[var(--peace-border-strong)] bg-white p-6 text-[var(--peace-muted)]">Non risultano prenotazioni scuola collegate a questa email.</p> : catalog.bookings.map((booking) => {
