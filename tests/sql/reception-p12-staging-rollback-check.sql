-- P12 staging only: existing synthetic group-panel fixture. No persistent test writes.
begin;
...
do $$ declare c record; v jsonb; request_id uuid:=gen_random_uuid(); revision integer; baseline_panels text; baseline_intentions text; begin
  ...
  begin perform public.reception_event_check_in('panel_entry',c.event_id,c.actor_id,'code',c.public_code);
    raise exception 'Panel duty accepted'; exception when insufficient_privilege then null; end;
  begin perform public.reception_event_check_in('room_assistance',c.event_id,c.actor_id,'code',c.public_code);
    raise exception 'Room duty accepted'; exception when insufficient_privilege then null; end;
  v:=public.reception_event_check_in('event_entry',c.event_id,c.actor_id,'qr',c.token_hash);
  assert v->>'status'='valid' and v->>'kind'='family';
  assert v::text !~ 'birth|email|phone|token|questionnaire|notes';
  begin perform public.reception_event_check_in('event_entry',c.event_id,gen_random_uuid(),'code',c.public_code); raise exception 'Foreign actor accepted';
  exception when insufficient_privilege then null; end;
  assert public.reception_event_check_in('event_entry',c.event_id,c.actor_id,'qr',repeat('0',64))->>'status'='invalid';
  v:=public.reception_event_check_in('event_entry',c.event_id,c.actor_id,'qr',c.token_hash,'enter',request_id,array[c.child_id]);
  assert v->>'status'='valid';
  assert (select count(*)=1 from public.check_ins where child_id=c.child_id and cancelled_at is null);
  assert public.reception_event_check_in('event_entry',c.event_id,c.actor_id,'qr',c.token_hash,'enter',request_id,array[c.child_id])->>'outcome'='replayed';
  revision:=(v->>'revision')::integer;
  assert public.reception_event_check_in('event_entry',c.event_id,c.actor_id,'code',c.public_code,'correct',gen_random_uuid(),array[c.registration_id],null,null,revision+1,'selection_error')->>'status'='conflict';
  v:=public.reception_event_check_in('event_entry',c.event_id,c.actor_id,'code',c.public_code,'correct',gen_random_uuid(),array[c.registration_id],null,null,revision,'selection_error');
  assert (select cancelled_at is not null from public.check_ins where child_id=c.child_id);
  revision:=(v->>'revision')::integer;
  v:=public.reception_event_check_in('event_entry',c.event_id,c.actor_id,'code',c.public_code,'cancel',gen_random_uuid(),array[c.registration_id],null,null,revision,'entry_cancelled');
  ...
  v:=public.reception_event_check_in('event_entry',c.event_id,c.actor_id,'qr',encode(sha256(convert_to(c.booking_id::text,'UTF8')),'hex'),'enter',gen_random_uuid(),'{}',8,1);
  assert v->>'students'='8' and v->>'companions'='1';
  v:=public.reception_event_check_in('event_entry',c.event_id,c.actor_id,'qr',encode(sha256(convert_to(c.booking_id::text,'UTF8')),'hex'),'correct',gen_random_uuid(),'{}',9,2,(v->>'revision')::integer,'count_error');
  assert v->>'students'='9';
  v:=public.reception_event_check_in('event_entry',c.event_id,c.actor_id,'qr',encode(sha256(convert_to(c.booking_id::text,'UTF8')),'hex'),'cancel',gen_random_uuid(),'{}',null,null,(v->>'revision')::integer,'entry_cancelled');
  assert v->>'students'='0';
...
reset role;
select 'PASS P12 staging duty scope, RPC, family, school, retry, correction conflict, cancellation, actor scope, audit and preserved intentions/panels';
rollback;
