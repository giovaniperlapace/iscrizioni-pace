import { resolveStatisticsReport } from "@/lib/registrations/statistics-reports";
...
const statistics = buildEventStatisticsSnapshot({groups,participants:["roma","trastevere"].map((id,i)=>({registrationId:`r${i}`,eventId:"event",eventTitle:"Evento",currentGroupId:id,currentGroupName:id,country:"Francia",city:"Parigi",childrenCount:i?2:0})),attendanceChoices:[{registration_id:"r1",day:"2026-10-24",day_part:"afternoon",choice:"yes"},{registration_id:"r1",day:"2026-10-25",day_part:"morning",choice:"yes"}],eventStartsOn:"2026-10-25",eventEndsOn:"2026-10-27"});
export default async function Fixture({searchParams}:{searchParams:Promise<{dashboard?:string;report?:string}>}) {
  const {dashboard,report}=await searchParams;
  return <main className="mx-auto min-w-0 max-w-6xl p-4"><StatisticsSection statistics={statistics} report={resolveStatisticsReport(report)} dashboard={dashboard==="admin"?"admin":"manager"} navMode="full" /></main>;
}
