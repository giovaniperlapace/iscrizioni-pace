...
const ab = (...args) => execFileSync("npx", ["--yes", "agent-browser", "--session", "manager-progress", ...args], { encoding: "utf8", timeout: 60000 });
...
try {
  ab("open", `${base}/pending-feedback-check?manager=1`);
  console.log(ab("snapshot", "-i"));
  check('document.querySelector("[data-save][data-button-progress=true]") && !document.querySelector("[data-nextjs-dialog]")', "Manager layout enables feedback without framework errors");
  ...
  run('[data-portal-save]', true, "portal inherits Manager feedback");
  ...
  ab("eval", 'document.querySelector("[data-reject]").click()');
  ab("click", "[data-leave]");
  check('!document.querySelector("[data-button-progress]")', "leaving Manager removes experimental feedback");
  check(`(async () => {
    const b = document.querySelector('[data-fail]'); b.click();
    await new Promise(r => setTimeout(r, 400));
    return getComputedStyle(b, '::after').content !== 'none' && !b.querySelector('.button-progress-overlay');
  })()`, "other dashboards retain the original spinner");
